...
def create_client(api_key: Optional[str] = None, base_url: str = BASE_URL) -> OpenAI:
    """
    ...
    if api_key is None:
        ...
        if not api_key:
            # Fallback for tests if env not set, though user wants to remove hardcoded
            # raising error might be better or leaving as None if client handles it
            pass
