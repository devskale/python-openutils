...
class InternLMProvider(ChatProvider):
    ...
    def list_models(cls, api_key: Optional[str] = None) -> list:
        ...
        if HAS_OPENAI:
            try:
                ...
                raise map_provider_error("InternLM", e, status_code=status_code, response_body=str(response_body) if response_body else None)
        else:
            try:
                headers = {
    ...
    def _complete_with_requests(
        ...
        **provider_specific_kwargs
        ...
        payload = {
            # Default to internlm3-latest if no model specified
            "model": request.model or "internlm3-latest",
            "messages": [{"role": msg.role, "content": msg.content} for msg in request.messages],
            "temperature": request.temperature,
            "stream": False,
        ...
        if response.status_code != 200:
            ...
            raise map_provider_error("InternLM", Exception(error_msg), status_code=response.status_code, response_body=response.text)
