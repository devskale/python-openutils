...
- **Thinking dialect** — a provider's native reasoning knob. Owned by the
  ...
  to its own dialect.
- **TTSTarget / STTTarget** — the audio dispatch modules (`uniinfer.audio`). The TTS/STT analogs of `Target` (chat) and `ImageTarget` (images): each binds a `provider@model` + API key, resolves the TTS/STT provider class from a lazy registry (`_TTS_REGISTRY` / `_STT_REGISTRY`), and owns request building + dispatch behind one small interface (`asynthesize` / `atranscribe`). Returns library-level result types (`TTSResult` / `STTResult`) that the router shapes into HTTP responses. Adding a new TTS/STT provider is a registry entry, not a router branch.
